...
if (!exists("summary_back")) {
  ...
  var_list <- c("paraphrase", "part_para")
  rm(list = var_list)
...
var_list = c("CP", "groups", "group_list", "i", "name", "CP_diff", "group")
rm(list=var_list)
```
...
- Without including variation, theories of language processing are limited in their ability to explain everyday speech
...
- Syntactic processing research with EEG typically relies on written stimuli containing grammatical violations or ambiguities
...
## Both dialect groups are sensitive to syntactic variation at the structure-building (Early AN) and integration (P600) levels of processing
...
- The interaction between dialect group and Southern cultural affiliation, but not language attitudes, significantly correlated with acceptability ratings of attested double modals
